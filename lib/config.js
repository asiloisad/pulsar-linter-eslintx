...
function initContext(config) {
  ...
  _CONTEXT.RESETENGINE =
    config.packagename + _STRINGS.DOT + _STRINGS.RESETENGINE;
  _CONTEXT.GRAMMARSCOPES =
    config.packagename + _STRINGS.DOT + _STRINGS.GRAMMARSCOPES;
  _CONTEXT.BASECONFIG = config.packagename + _STRINGS.DOT + _STRINGS.BASECONFIG;
  _CONTEXT.ESLINTCONFIG =
    config.packagename + _STRINGS.DOT + _STRINGS.ESLINTCONFIG;
} /* eslint-disable-next-line no-unused-vars */

/**
 *  Returns ESLint options and reset status.
 *
 *  This is about 'linter-eslint's 'configSchema.eslintopts' (package.json), which is getting
 ...
 */ function getOptions(atom, filepath, engine) {
  return new Promise((resolve) => {
    ...
    if (
      ...
      const projectpath = _m.util.getProjectPath(
        filepath,
        atom.project.rootDirectories
      );
      const baseconfig = JSON.parse(
        JSON.stringify(atom.config.get(_CONTEXT.BASECONFIG) || {})
      );
      const eslintconfig = JSON.parse(
        JSON.stringify(atom.config.get(_CONTEXT.ESLINTCONFIG) || {})
      );
      ...
      // File enumeration
      if (eslintconfig.cwd) {
        options.cwd = eslintconfig.cwd;
      }
      // options.ignorePath - not yet supported

      // Linting
      options.allowInlineConfig =
        eslintconfig.allowInlineConfig === false ? false : true;

      if (baseconfig.extends && baseconfig.extends.length < 1) {
        delete baseconfig.extends;
      }
      options.baseConfig =
        Object.getOwnPropertyNames(baseconfig).length > 0 ? baseconfig : null;
      ...
      options.reportUnusedDisableDirectives = !reportunuseddtives
        ? null
        : reportunuseddtives;

      const rulepaths = eslintconfig.rulePaths;
      options.rulePaths = _m.util.buildRulePaths(
        rulepaths,
